...
while read -ra a ;
  do
    ...
    then
      cutadapt  --report=full -a AGATCGGAAGAGCACACGTCT -j 16 \
      -o data/$seqtype/preproccessed_data/trimmed/${a[0]} data/$seqtype/raw_data/${a[0]}
    else
      cutadapt  --discard-untrimmed -m 20 --report=full -a AGATCGGAAGAGCACACGTCT -j 16 \
      -o data/$seqtype/preproccessed_data/trimmed/${a[0]} data/$seqtype/raw_data/${a[0]}
    fi
    # filter rRNA
    $star_path/STAR \
...
for seqtype in riboseq_chx riboseq_harr riboseq_nd
  do
  mkdir -p data/$seqtype/mapped/merged
