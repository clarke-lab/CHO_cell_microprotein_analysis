...
ggsave(plot=new_orf_classification_plot, filename = paste(results_dir,"Figure_3.4_A.png", sep=""), dpi = 700, device = "png", width=3, height = 3) 
```
## Number of small ORFs per non-coding RNA
```{r}
lncRNA_smorfs <- table_s2 %>%
  filter(`ORF type` == "New" & 
...
ggsave(plot= new_ORFs_per_transcript_plot,filename = paste(results_dir,"Figure_3.4_B.png", sep=""), dpi = 700, device = "png", width = 3, height = 3) 
```
...
ggsave(plot = lncRNA_smorf_length_plot, filename = paste(results_dir,"Figure_3.4_C.png", sep=""), dpi = 700, device = "png", width = 3, height = 3) 
```
...
ggsave(plot = rnaseq_volcano_plot, filename = paste(results_dir, "Figure_3.5_B.png", sep = ""), width = 3, height = 4, device = "png", dpi = 700)
```
```{r}
...
combined_volcano
ggsave(plot = combined_volcano, filename = paste(results_dir, "Figure_3.6_S1.png", sep = ""), width = 6, height = 4, device = "png", dpi = 700)
```
...
ggsave(  plot=de_comparison, filename = paste(results_dir, "Figure_3.6_A.png", sep = ""),
  width = 5, height = 5, device = "png", dpi = 700)

volcano_venn <- combined_volcano / de_comparison + plot_layout(heights=unit(c(5,1), c("cm","null")))
  
ggsave(plot = volcano_venn, filename = paste(results_dir, "Figure_3.5_S6.png", sep = ""), width =9, height = 9, device = "png", dpi = 700)
...
## Volcano plot
```{r}
aggreement_changes_smorf <- c(intersect(RNASeq_up$geneid, RiboSeq_up$geneid), 
...
riboseq_cor_smorf_rnaseq_volcano_plot
  ggsave(plot = riboseq_cor_smorf_rnaseq_volcano_plot, filename = paste(results_dir, "Figure_3.6_B.png", sep = ""), width = 3, height = 4, device = "png", dpi = 700)
```
```{r}
...
boxplot_smorf <- bind_rows(
  ...
  mutate(condition = toupper(tcounts$condition),
...
boxplot_smorf 
  ggsave(plot =boxplot_smorf , filename = paste(results_dir, "Figure_3.6_S2.png", sep = ""), width =10, height = 10, device = "png", dpi = 700)
```
## DTE
```{r}
...
smorf_te_boxplots <- bind_rows(
  ...
  labs(x = "", y = expression("Log"[2] ~ "TE (TS/NTS)") , fill = "") +
  scale_fill_manual(values = c("#D55E00", "#56B4E9")) +
  theme(strip.text = element_text(face = "italic",size=8), strip.background = element_blank(), axis.title = element_text(size=6),legend.position = c(0.8,0.15), axis.title.y = element_text(size=12))
smorf_te_boxplots
ggsave(smorf_te_boxplots, filename = paste(results_dir, "Figure_3.5_F.png", sep = ""), width = 7, height = 5, device = "png", dpi = 700)
```
