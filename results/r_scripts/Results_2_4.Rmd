...
res_rnaseq_canonical <- res_rnaseq %>%
 ...
 res_rnaseq <- bind_rows(res_rnaseq_smorf, res_rnaseq_canonical)
...
rnaseq_volcano_plot_data <- res_rnaseq %>%
 #filter(!str_detect(geneid, "NR|XR")) %>%
  ...
  mutate(type = case_when(
    geneid %in% smorf_ids$`ORF-RATER name` & padj < pval_threshold & abs(log2FoldChange) >= log2(fold_change_threshold) ~ "sORF" ,
    !geneid %in% smorf_ids$`ORF-RATER name` & padj < pval_threshold & abs(log2FoldChange) >= log2(fold_change_threshold) ~ "Canonical" ,
    TRUE ~ "Non DE"
  )) %>%
  mutate(protein_alpha = case_when(grepl("XR_|XM_", geneid) & padj < pval_threshold & abs(log2FoldChange) >= log2(fold_change_threshold)  ~ 1,
  ...
  mutate(protein_size = case_when(grepl("XR_|XM_", geneid) & padj < pval_threshold & abs(log2FoldChange) >= log2(fold_change_threshold)  ~ 1,
...
res_riboseq_canonical <- res_riboseq %>%
 ...
 res_riboseq <- bind_rows(res_riboseq_smorf, res_riboseq_canonical)
...
range(res_riboseq$log2FoldChange)
```
...
RNASeq_up_smorf <- sig_res_rnaseq %>%
   filter(Plastid_ID %in% smorf_ids$`ORF-RATER name`) %>%
  filter(log2FoldChange > 0) %>%
  dplyr::select(Plastid_ID)
...
RiboSeq_up_smorf <- sig_res_riboseq %>%
   filter(Plastid_ID %in% smorf_ids$`ORF-RATER name`) %>%
  filter(log2FoldChange > 0) %>%
  dplyr::select(Plastid_ID)

RNASeq_up <- bind_rows(RNASeq_up_smorf, RNASeq_up_canonical)
RiboSeq_up <- bind_rows(RiboSeq_up_smorf, RiboSeq_up_canonical)
...
RNASeq_down_smorf <- sig_res_rnaseq %>%
   filter(Plastid_ID %in% smorf_ids$`ORF-RATER name`) %>%
  filter(log2FoldChange < 0) %>%
  dplyr::select(Plastid_ID)
...
RiboSeq_down_smorf <- sig_res_riboseq %>%
   filter(Plastid_ID %in% smorf_ids$`ORF-RATER name`) %>%
  filter(log2FoldChange < 0) %>%
  dplyr::select(Plastid_ID)

RNASeq_down <- bind_rows(RNASeq_down_smorf, RNASeq_down_canonical)
RiboSeq_down <- bind_rows(RiboSeq_down_smorf, RiboSeq_down_canonical)

dim(RiboSeq_down_canonical)[1] + dim(RiboSeq_up_canonical)[1] 
dim(RiboSeq_down_smorf)[1] + dim(RiboSeq_up_smorf)[1]
...
range(riboseq_cor_rnaseq_volcano_plot$log2FoldChange)
```
...
```{r}
fig_5_ab <- ggarrange(riboseq_cor_rnaseq_volcano_plot, ggplot() +
  theme_void(), te_scatter_plot, ncol = 3, widths = c(1, 0.05, 1))
fig_5_ab

ggsave(plot = fig_5_ab, filename = paste(results_dir, "Figure_5_A_B.png", sep = ""), width = 11, height = 4, device = "png", dpi = 700)
```
...
ggsave(enrichment_plot_all, filename = paste(results_dir, "Figure_5C.png", sep = ""), width = 5, height = 5, device = "png", dpi = 700)
```
# DNA repair genes
...
dna_repair_boxplots
ggsave(dna_repair_boxplots, filename = paste(results_dir, "Figure_5D.png", sep = ""), width = 6, height = 6, device = "png", dpi = 700)
```
...
brca1_plot
ggsave(brca1_plot, filename = paste(results_dir, "Figure_5E.png", sep = ""), width = 12, height = 6, device = "png", dpi = 700)
```
