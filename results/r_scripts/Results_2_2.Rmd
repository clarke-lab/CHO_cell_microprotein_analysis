...
table(table_s2$`ORF type`, table_s2$`Start codon`)
```
# Figure 2 plots
## Summary plot
```{r message=FALSE, warning=FALSE}
orftype_codon_data <- data.frame(table(table_s2$`ORF type`, table_s2$`Start codon`)) %>%
...
summary_start <- table(table_s2$`ORF type`, table_s2$`Start codon`)
...
figure_s2 <- ggarrange(uorf_length_plot,
          ggplot()+theme_void(),
          ouorf_length_plot,ggplot()+theme_void(),
                         new_orf_length_plot,
                         ncol=5,  widths = c(1,0.05,1,0.05,1))
ggsave(plot =figure_s2, filename = paste(results_dir,"Figure_S3.png", sep=""), dpi = 700, device = "png", width = 9, height = 3) 
```
...
current_orf <- orftable_filtered %>%
  filter(tid == selected_transcript) %>%
  filter(orftype == "Extension" | orftype == "Annotated")
current_orf

harr_track_data <- bedgraph_data %>%
  filter(X1 == selected_transcript) %>%
  filter(ribotype == "harr-nd") %>%
  filter(X2 <= max(current_orf$tstop) & X2 >= 0) %>%
...
track_data <- track_data %>%
  mutate(frame="NA") %>%
  mutate(frame=ifelse((X2 >= min(current_orf$tcoord) & X2 <= current_orf$tstop[2] & ribotype == "chx" ), (X2 - current_orf$tcoord) %% 3, NA))
...
harr_plot <- track_data %>%
 ...
 add_row(X1 = selected_transcript, X2 =0, X3 =1,X4=0, ribotype = "harr-nd", corrected=0, scale_bpm=0, .before = 1) %>%
  ...
  facet_zoom(xlim = c(min(current_orf$tcoord) - 10, max(current_orf$tcoord) + 10), zoom.size = 1, split = F) +
  labs(
...
ggsave(filename = paste(results_dir,"Figure_3.2_B_harr.png", sep=""), width = 6, height = 4, device = "png", dpi = 700)
...
chx_plot <- track_data %>%
  ...
  facet_zoom(xlim =  c(min(current_orf$tcoord) - 10, max(current_orf$tcoord) + 10), shrink = FALSE, zoom.size = 1) +
  labs(
    x = "Transcript coordinates (nts)",
    y = "Ribo-seq (Scaled BPM)",
    title = "",
...
ggsave(plot=chx_plot,filename = paste(results_dir,"Figure_3.2_B_chx.png", sep=""), width = 6, height = 4, device = "png", dpi = 700)
