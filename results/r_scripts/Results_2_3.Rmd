...
## uORFs length distribution
```{r}
uorf_length_plot <- uorfs %>%
...
geom_text_npc(aes(npcx=0.9, npcy=0.9,label=paste0("Mean length:\n",round(mean(uorfs$`Length (AAs)`)), " aa")),size=3)+
theme_bw() 
uorf_length_plot

#ggsave(plot=uorf_length_plot, filename= paste(results_dir,"Figure_3.3_B.png", sep=""), width = 3, height = 3, device='png', dpi=700)
```
...
## ouORFs length distribution
```{r}
ouorf_length_plot <- ouorfs %>%
  ggplot(aes(x=sort(`Length (AAs)`))) +
 ...
 # annotate("text", x = 320, y=140, size=1.5, label=paste0("Mean length:\n",round(mean(ouorfs$`Length (AAs)`)), " aa")) +
  ...
  theme_bw() + 
geom_text_npc(aes(npcx=0.9, npcy=0.9,label=paste0("Mean length:\n",round(mean(ouorfs$`Length (AAs)`)), " aa")),size=3)
ouorf_length_plot 
#ggsave(plot= ouorf_length_plot, filename= paste(results_dir,"Figure_3.3_D.png", sep=""), width = 3, height =3, device='png', dpi=700)
```
...
table(ouORF_te_data_plot$class)
...
ouORF_te_density_plot <- ouORF_te_data_plot  %>%
  ...
  labs(color="", y="Cumlative proportion", x=expression("Main ORF CDS TE (Log"[2]  * ")")) + 
  theme(legend.position = "none" ) +
  annotation_custom(orf_stat_table, xmin = 0.55, xmax = 1, ymin = 0.96, ymax =1)
...
table(ouORF_rna_data_plot$class)
...
single_atg_uorf_plot_te <- single_atg_uorf_data %>% 
  ...
  labs(color="", y="Cumlative proportion", x="RNA abundance (Log2 RPKM)") + 
  theme(legend.position = "none" ) + 
...
joined_plot_S3 <- ggarrange(single_atg_uorf_plot_te,
                         ggplot()+theme_void(),
...
# main figure
```{r}
joined_plot <- ggarrange(uorf_per_transcript_plot,
                         ggplot()+theme_void(),
                         uORF_te_density_plot, 
                         ggplot()+theme_void(),
...
ggsave(plot= joined_plot, filename= paste(results_dir,"Figure_3.3_G.png", sep=""), width = 9, height = 6, device='png', dpi=700)
```
### Supplementary Figure 3

```{r}
joined_plot <- ggarrange(uORF_rna_density_plot, 
                         ggplot()+theme_void(),
...
ggsave(plot= joined_plot, filename= paste(results_dir,"Figure_3.3_S3.png", sep=""), width = 9, height = 6, device='png', dpi=700)
```
...
## Load bedgraph data
```{r include=FALSE}
chx_bedgraph <- read_tsv("/mnt/HDD2/colin/ribosome_footprint_profiling/alignment_tracks/merged/riboseq_chx.p.site.transcriptome.bedgraph",
  col_names = F
) %>% mutate(ribotype = "chx")
...
chx_full_bedgraph <- read_tsv("/mnt/HDD2/colin/ribosome_footprint_profiling/alignment_tracks/merged/chx.fullcov.site.transcriptome_bs0.bedgraph",
  col_names = F
...
ggsave(filename = paste(results_dir,"Figure_3.3_I.png", sep=""), width = 6, height =3, device = "png", dpi = 700)
```
## Rad51d ouORF
```{r}
...
ggsave(filename = paste(results_dir,"Figure_3.3_J.png", sep=""), width = 6, height = 3, device = "png", dpi = 700)
```